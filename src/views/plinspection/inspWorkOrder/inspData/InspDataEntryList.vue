<template>
  <div class="insp-list">
    ...
    <el-table :data="list" border v-loading="loading">
      ...
      <el-table-column prop="itemName" label="物料名称" width="130" />
      ...
      <el-table-column prop="inspectReviewer" label="检验审核人" width="100" />
      <!-- 检验完成时间 -->
       <el-table-column prop="inspectFinishTime" label="检验完成时间"  />


      <el-table-column label="操作" width="350" fixed="right"> <!-- 加宽操作列以容纳三个按钮 -->
        <template #default="{ row }">
          <!-- 状态20（检验中）显示原有两个按钮 -->
          <el-button v-if="row.status == 20 || row.status == 23" size="small" type="success" @click="updateStatus(row, 21)">提交审核</el-button>          
          <!-- <el-button v-if="row.status == 20" size="small" type="warning" @click="editInspectionResult(row)">编辑检验结果</el-button> -->
          <el-button v-if="row.status == 20 || row.status == 23" size="small" type="warning" @click="editInspectionResult(row)">录入检验结果</el-button>
...
const baseStatusOptions = [
  ...
  { label: '检验不通过', value: 23 } // 21及其他状态默认显示"检验完成"
