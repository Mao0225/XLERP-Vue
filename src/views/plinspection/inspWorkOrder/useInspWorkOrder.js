...
import { getInspWorkOrderPage,updateInspWorkOrderStatus,deleteInspWorkOrder} from '@/api/plinspection/inspWorkOrder'
...
export function useInspOrder(minStatus = null) {
  ...
  const getList = async () => {
    loading.value = true
    try {
      const res = await getInspWorkOrderPage(query)
  ...
  const updateStatus = async (row, newStatus, needRemark = false) => {
    ...
    try {
      await updateInspWorkOrderStatus({
        orderId: row.id,
        newStatus,
        remark,
  ...
  const handleDelete = (row) => {
    ElMessageBox.confirm(`确认删除 "${row.orderNo}"？`, '警告', { type: 'warning' })
      .then(async () => {
        await deleteInspWorkOrder({ id: row.id })
