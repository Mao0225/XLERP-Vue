      ...
      <div class="inbound-list">
        <el-card shadow="never">
          ...
          <el-table :data="inboundList" v-loading="loading" height="600" border>
            ...
            <el-table-column prop="transactionDate" label="出库日期" width="180" show-overflow-tooltip />
                ...
                <el-space>
                  <template v-if="row.status == 10">
                    ...
                    <el-button type="warning" size="small" @click="updateStatus(row.id, 20)">
                      <el-icon><CircleCheckFilled /></el-icon> 确认出库
                    </el-button>
                  </template>
...
import { getNewNoNyName } from '@/api/system/basno'
	...
	const getNewdocNo = async () => {
	  try {
	    const res = await getNewNoNyName('cl-ck');
...
const statusOptions = ref([
  ...
  { value: '出库完成', label: '出库完成', icon: 'CircleCheck' },
]);
...
const updateStatus = async (id, status) => {
  ...
  try {
    ...
    if (confirmed) {
      ...
      ElMessage.success(status === 10 ? '确认出库成功' : '撤销确认成功');
      getInboundListData();
    }
  } catch (error) {
    // 如果是用户取消操作，不显示错误信息
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(`确认删除出库单"${row.docNo}"吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    });
