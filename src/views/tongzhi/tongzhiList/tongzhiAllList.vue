<template>
  <div class="notice-management">
          ...
          <el-table :data="noticeList" :highlight-current-row="true" v-loading="loading" height="600" border>
            ...
            <el-table-column label="操作" width="330" fixed="right">
              <template #default="{ row }">
                ...
                <template v-if="row.noticestatus === 31">
    ...
    <MaterialList :visible="showMaterialListDialog" :contract-no="selectedContractNo" :contract-name="selectedContractName" @update:visible="showMaterialListDialog = $event" :noticeid="selectedNoticeid"  />
  </div>
</template>
...
const openMaterialList = (row) => { 
    selectedContractNo.value = row.contractNo;
selectedContractName.value = row.contractName;
selectedNoticeid.value = row.noticeid;
