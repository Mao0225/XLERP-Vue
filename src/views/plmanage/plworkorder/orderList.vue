<template>
  <div class="production-work-orders">
    ...
    <div class="main-content">
      <!-- Left: Production Work Orders List -->
      ...
      <div class="order-detail">
        <el-card shadow="never" class="detail-card">
            ...
            <div class="detail-section">
              ...
              <div class="info-grid">
                ...
                <div class="info-item">
                  ...
                  <span class="value">{{ selectedOrder.materialsDescription || '无' }}</span>
                </div>
                <div class="info-item">
                  <span class="label">产品型号规格:</span>
                  <span class="value">{{ selectedOrder.modelSpec || '无' }}</span>
                </div>
                <div class="info-item">
                  <span class="label">实物ID:</span>
                  <span class="value">{{ selectedOrder.entityCode || '无' }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="detail-section">
              ...
              <div class="info-grid">
                <div class="info-item">
                  ...
                  <span class="value">{{ selectedOrder.processRouteNo || '无' }}</span>
                </div>
                <div class="info-item">
                  <span class="label">电压等级:</span>
                  <span class="value">{{ selectedOrder.voltageLevel || '无' }}</span>
                </div>
                <div class="info-item">
                  <span class="label">数据来源:</span>
                  <span class="value">{{ selectedOrder.dataSource }}</span>
                </div>
              </div>
            </div>
          ...
          <el-empty v-else description="请选择一个生产工单查看详情" />
        </el-card>
      </div>
    </div>
...
import { getPlWorkOrderList, deletePlWorkOrder, getPlWorkOrderById } from '@/api/plmanage/plworkorder';
...
const displayOrders = computed(() => {
  ...
  if (filters.woNo) {
    filtered = filtered.filter(item =>
      item.woNo.toLowerCase().includes(filters.woNo.toLowerCase())
    );
  }
...
const deleteOrder = async (order) => {
  try {
    await ElMessageBox.confirm(`确认删除生产工单 ${order.woNo}？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    });

    const res = await deletePlWorkOrder({ id: order.id });
    if (res.code === 200 && res.success) {
