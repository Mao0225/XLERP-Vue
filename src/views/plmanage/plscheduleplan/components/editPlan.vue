...
watch(() => props.visible, (newVal) => {
  ...
  if (newVal) {
    // Ensure planPeriod and actualPeriod are numbers
    const initialData = { ...props.initialData }
    initialData.planPeriod = initialData.planPeriod ? Number(initialData.planPeriod) : null
    initialData.actualPeriod = initialData.actualPeriod ? Number(initialData.actualPeriod) : null
  ...
  planPeriod: [
    { required: true, message: '请输入计划工期', trigger: 'blur' },
    {
      validator: (rule, value, callback) => {
        if (value === null || value === '') {
          callback(new Error('请输入计划工期'))
        } else if (isNaN(Number(value))) {
          callback(new Error('计划工期必须为数字'))
        } else {
          callback()
        }
      },
  ...
  formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        const submitData = {
          ...form,
          planPeriod: form.planPeriod ? Number(form.planPeriod) : null,
          actualPeriod: form.actualPeriod ? Number(form.actualPeriod) : null
        }
        const response = await updatePlSchedulePlan(submitData)
