...
import { getLhjxPage, deleteLhjx, getLhjxById, updateStatus } from '@/api/clmanage/cl-lhjx'
...
const generateNewCode = async () => {
  try {
    const res = await getNewNoNyName('cl-lhjx')
    if (res?.code === 200) {
...
const handleDelete = async (row) => {
  try {
    ...
    const response = await deleteLhjx({ id: row.id })
    if (response?.code === 200) {
