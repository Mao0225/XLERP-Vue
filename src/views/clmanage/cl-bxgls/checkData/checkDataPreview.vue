      ...
      <el-row :gutter="16">
          ...
          <el-col :span="24">
            <el-row :gutter="16">
              <el-col :span="6" v-for="(item, index) in [
                { key: 'shapeStandard', label: '外形规格' },  
                { key: 'shapeOutOfRoundness', label: '外形不圆度' },  
                { key: 'shapeDeterminationResult', label: '外形判断结果' } 
              ]" :key="index">  
        ...
        <el-col :span="24">
          ...
          <el-col :span="24">
            <el-row :gutter="16">
              <el-col :span="6" v-for="(item, index) in [
                { key: 'sizeStandard', label: '尺寸规格' },  
                { key: 'sizeRequired1', label: '尺寸实际测量值一' },  
                { key: 'sizeRequired2', label: '尺寸实际测量值二' },
                { key: 'sizeRequired3', label: '尺寸实际测量值三' },
                { key: 'sizeMean', label: '尺寸平均值' },
                { key: 'sizeDeterminationResult', label: '尺寸判断结果' }  
              ]" :key="index">  
                <el-form-item :label="item.label">
                  <span>{{ initialData[item.key] || '-' }}</span>
                </el-form-item>
        ...
        <el-col :span="24">
          ...
          <el-col :span="24">
            <el-row :gutter="16">
              <el-col :span="6" v-for="(item, index) in [
                { key: 'chemStandard', label: '化学成分规格' },  
                { key: 'chemBatchNo', label: '化学成分炉号' },  
                { key: 'chemBatchNum', label: '化学成分批号' } 
              ]" :key="index">  
                <el-form-item :label="item.label">
                  <span>{{ initialData[item.key] || '-' }}</span>
                </el-form-item>
          ...
          <el-col :span="24">
            <el-row :gutter="16">
              <el-col :span="6" v-for="(item, index) in [
                { key: 'chemC', label: 'C的实测值' },  
                { key: 'chemSi', label: 'Si的实测值' },  
                { key: 'chemMn', label: 'Mn的实测值' },
                { key: 'chemCr', label: 'Cr的实测值' },
                { key: 'chemNi', label: 'Ni的实测值' },
              ]" :key="index">  
                <el-form-item :label="item.label">
                  <span>{{ initialData[item.key] || '-' }}</span>
                </el-form-item>
          ...
          <el-col :span="24">
            <el-row :gutter="16">
              <el-col :span="6" v-for="(item, index) in [
                { key: 'chemDeterminationResult', label: '化学成分判断结果' }
              ]" :key="index">  
                <el-form-item :label="item.label">
                  <span>{{ initialData[item.key] || '-' }}</span>
                </el-form-item>
