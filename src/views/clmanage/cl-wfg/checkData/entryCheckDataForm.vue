  ...
  >
    <el-form ref="formRef" :model="form" :rules="rules" label-width="120px" class="custom-form">
      <el-row :gutter="16">
        ...
        <el-col :span="12">
          <el-form-item label="力学性能检验抽检数量(件)" prop="mechInspQty">
                ...
                <el-row :gutter="8">
                  <el-col :span="12">
                    <el-form-item :prop="chem.actualProp" :rules="rules[chem.actualProp]">
                      <el-input v-model="form[chem.actualProp]" :placeholder="chem.label + '实测值'" clearable size="small" />
                    </el-form-item>
                  </el-col>
                  <el-col :span="12">
...
const form = reactive({
  ...
  tensileStrength2: '',
  tensileStrength3: '',
  tensileStrengthRequired: '≥410',
  yieldStrength1: '',
  yieldStrength2: '',
  yieldStrength3: '',
  yieldStrengthRequired: '≥245',
  elongation1: '',
  elongation2: '',
  elongation3: '',
  elongationRequired: '≥25',
  // 其他字段
  leaveFactoryDate: '',
  detectionTime: '',
  status: '40',
...
const rules = reactive({
  ...
  // 化学成分验证规则
  chemC: [
    // { required: false, message: '请输入C含量', trigger: 'blur' },
    // { type: 'number', message: '必须为数字', trigger: 'blur' }
    { max: 20, message: '长度不能超过20个字符', trigger: 'blur' }
  ],
  chemSi: [
    // { required: false, message: '请输入Si含量', trigger: 'blur' },
    // { type: 'number', message: '必须为数字', trigger: 'blur' }
    { max: 20, message: '长度不能超过20个字符', trigger: 'blur' }
  ],
  chemMn: [
    // { required: false, message: '请输入Mn含量', trigger: 'blur' },
    // { type: 'number', message: '必须为数字', trigger: 'blur' }
    { max: 20, message: '长度不能超过20个字符', trigger: 'blur' }
  ],
  chemCu: [
  ...
  chemS: [
    // { required: false, message: '请输入S含量', trigger: 'blur' },
    // { type: 'number', message: '必须为数字', trigger: 'blur' }
    { max: 20, message: '长度不能超过20个字符', trigger: 'blur' }
  ],
  chemCRequired: [
...
watch(() => props.initialData, (newData) => {
  if (newData && Object.keys(newData).length > 0) {
    Object.assign(form, {
      ...
      tensileStrengthRequired: newData.tensileStrengthRequired || '≥410',
      yieldStrength1: newData.yieldStrength1 || '',
      yieldStrength2: newData.yieldStrength2 || '',
      yieldStrength3: newData.yieldStrength3 || '',
      yieldStrengthRequired: newData.yieldStrengthRequired || '≥245',
      elongation1: newData.elongation1 || '',
      elongation2: newData.elongation2 || '',
      elongation3: newData.elongation3 || '',
      elongationRequired: newData.elongationRequired || '≥25',
      // 其他字段
