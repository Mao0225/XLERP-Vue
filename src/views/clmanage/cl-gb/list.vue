<template>
  <div class="steelplate-ingot-management">
    <div class="action-bar">
      <el-input
        ...
        clearable
        @clear="getSteelplateIngotList"
        @keyup.enter="getSteelplateIngotList"
      />
      <el-input
      ...
      <el-input
        ...
        @keyup.enter="getSteelplateIngotList"
      />
      <el-button type="primary" @click="getSteelplateIngotList">搜索</el-button>
    ...
    <el-table :data="steelplateIngotList" border v-loading="loading" style="width: 100%">
      ...
      <el-table-column prop="weight" label="重量" width="90">
        <template #default="{ row }">
          <el-tooltip :content="row.weight" placement="top">
            <span class="truncate">{{ row.weight }} t</span>
          </el-tooltip>
        </template>
      </el-table-column>
      <el-table-column prop="size" label="尺寸(mm)" width="90">
        <template #default="{ row }">
          <el-tooltip :content="row.size" placement="top">
            <span class="truncate">{{ row.size }} t</span>
          </el-tooltip>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="chemNb" label="Nb(%)" width="90" />
      <el-table-column prop="chemV" label="V(%)" width="90" />
      ...
      <el-table-column prop="chemMo" label="Mo(%)" width="90" />
...
import { getGbPage, deleteGb, getGbById } from '@/api/clmanage/cl-gb'
...
const generateNewCode = async () => {
  try {
    const res = await getNewNoNyName('cl-gb');
...
const handleEdit = async (id) => {
  const res = await getGbById({ id: id })
...
const handleSuccessAdd = () => {
  ...
  ElMessage.success('钢板记录新增成功')
  getSteelplateIngotList()
}

const handleSuccessEdit = () => {
  editDialogVisible.value = false
  ElMessage.success('钢板记录修改成功')
  getSteelplateIngotList()
}
...
const steelplateIngotList = ref([])
const total = ref(0)
const loading = ref(false)

const getSteelplateIngotList = async () => {
  loading.value = true
  try {
    const res = await getGbPage(queryParams)
    steelplateIngotList.value = res.data.page.list
    total.value = res.data.page.totalRow
  } catch (error) {
    console.error('获取钢板列表失败', error)
    ElMessage.error('获取钢板列表失败')
  } finally {
    loading.value = false
  }
}
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除钢板记录"${row.basno}"吗？`, '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      await deleteGb({ id: row.id })
      ElMessage.success('钢板记录删除成功')
      getSteelplateIngotList()
    } catch (error) {
      console.error('删除钢板记录失败', error)
      ElMessage.error('删除钢板记录失败')
    }
  }).catch(() => {})
}
