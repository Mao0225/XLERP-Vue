  ...
  <div class="steelplate-ingot-management">
    <div class="action-bar">
      ...
      <div class="search-inputs">
          ...
          clearable @clear="getSteelplateIngotList" @keyup.enter="getSteelplateIngotList" />
      ...
      <el-table-column prop="matMaterial" label="钢板牌号" width="120">
        <template #default="{ row }">
      ...
      <el-table-column prop="mechelongation1" label="延伸率1##" width="90" />
      <el-table-column prop="mechelongation2" label="延伸率2#" width="90" />
      <el-table-column prop="mechelongation3" label="延伸率3#" width="90" />
      
      <el-table-column label="冲击实验" width="200">
  <template #default="{ row }">
    <el-tooltip :content="`1#: ${row.impactexp1 || '-'} 2#: ${row.impactexp2 || '-'} 3#: ${row.impactexp3 || '-'}`" placement="top">
      <span class="truncate">
        {{ row.impactexp1 || '-' }}, {{ row.impactexp2 || '-' }}, {{ row.impactexp3 || '-' }}
      </span>
    </el-tooltip>
  </template>
</el-table-column>
...
import { getGbPage, getGbById, updateStatus } from '@/api/clmanage/cl-gb'
...
const handleStatusUpdate = async (orderId, targetStatus) => {
  try {
    ...
    if (response?.code === 200) {
      ...
      getSteelplateIngotList() // 重新加载表格数据
    } else {
