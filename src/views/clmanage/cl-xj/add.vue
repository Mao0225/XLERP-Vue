<template>
  <el-dialog title="新增橡胶检验记录" :model-value="visible" width="1300px" :center="true" :close-on-click-modal="false"
    ...
    <!-- 底部按钮区域 - 移到el-form外面，作为el-dialog的直接子元素 -->
    <template #footer>
...
const submitForm = async () => {
  ...
  try {
    ...
    // 设置当前时间为录入时间
    form.writeTime = new Date().toISOString().split('T')[0]
    
    const res = await createXj(form)
    if (res.code === 200) {
    ...
    console.error('新增橡胶检验记录失败', error)
    if (error.name !== 'Error') {
      ElMessage.error('表单验证失败，请检查输入')
    } else {
      ElMessage.error('新增橡胶检验记录失败')
    }
  } finally {
...
const resetForm = () => {
  ...
  Object.assign(form, {
    ...
    mafactoryno: '',
    mafactoryname: '',
    detectionTime: getCurrentDate(), // 重置时也设置为当前日期
    matname: '',
    material: '',
    mattype: '',
    testno: '',
