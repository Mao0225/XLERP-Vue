<template>
  <div class="jxjjLbgx-management">
    ...
    <el-input v-model="queryParams.matRecheckNo" placeholder="请输入来料检验批次号" style="width: 200px; margin-right: 10px;"
      ...
      <el-button type="primary" @click="getJxjjLbgxList">搜索</el-button>
    ...
    <el-table :data="jxjjLbgxList" border v-loading="loading" style="width: 100%">
      <!-- 序号 -->
...
<el-table-column prop="writeTime" label="录入时间" />
      ...
      <el-tooltip :content="file.name" placement="top">
            <span class="file-link" @click="openFileInNewWindow(file.url, file.name)">打开文件</span>
           </el-tooltip> 
          </div>
        </template>
      </el-table-column>
  ...
  />
            ...
            <el-form-item label="力学性能-单丝强度" prop="singleWireStrength">
              <el-input v-model.number="form.singleWireStrength" placeholder="请输入力学性能-单丝强度" type="number" />
            </el-form-item>
...
import { getJxjjLbgx, getJxjjLbgxById, createJxjjLbgx, updateJxjjLbgx, deleteJxjjLbgx , getWoNoList , getBasNo } from '@/api/clmanage/jxjjlbgx'
...
// 获取数据列表
const getJxjjLbgxList = async () => {
  loading.value = true
  try {
    const res = await getJxjjLbgx(queryParams)
    jxjjLbgxList.value = res.data.page.list
...
const handleAdd = async() => {
  ...
  try {
    ...
    if (nextBasNo.value) {
      ...
      const res = await getBasNo('jxjjlbgx') // 注意：这里的参数可能需要根据实际业务调整
      if (res.code === 200) {
...
const handleEdit = async (row) => {
  ...
  const res = await getJxjjLbgxById({ id: row.id })
  Object.assign(form, res.data.jxjjLbgx)
   if (!form.writer) {
    form.writer = userStore.descr || '未知用户'
  }
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除数据"${row.no}"吗？`, '提示', {
    ...
    try {
      await deleteJxjjLbgx({ id: row.id })
