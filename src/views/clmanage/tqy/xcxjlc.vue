...
const handleWoNoSelectorSelect = (woNoInfo) => {
  ...
  if (woNoInfo.woStatus) {
    form.woStatus = woNoInfo.woStatus;
    console.log('设置工单状态:', form.woStatus); // 打印设置的生产订单号
  } else {
    ElMessage.warning('未找到对应的工单状态');
  }

  if (woNoInfo.purchaserHqCode) {
    form.purchaserHqCode = woNoInfo.purchaserHqCode;
    console.log('设置采购方总部编码:', form.purchaserHqCode); // 打印设置的生产订单号
  } else {
    ElMessage.warning('未找到对应的采购方总部编码');
  }
  ...
  if (woNoInfo.contractNo) {
   ...
   /* // 新增字段赋值
  form.purchaserHqCode = woNoInfo.purchaserHqCode || '';
  form.woStatus = getStatusLabel(woNoInfo.woStatus) || ''; */
