<template>
  <div class="jgbLc-management">
    ...
    <el-input v-model="queryParams.matRecheckNo" placeholder="请输入来料检验批次号" style="width: 200px; margin-right: 10px;"
      ...
      <el-button type="primary" @click="getFzcLcList">搜索</el-button>
    ...
    <el-table :data="jgbLcList" border v-loading="loading" style="width: 100%">
      <!-- 序号 -->
...
<el-table-column prop="writeTime" label="录入时间" />
      ...
      <el-table-column prop="chemCr" label="化学成分分析-Cu" />
...
import { getJgbLc, getJgbLcById, createJgbLc, updateJgbLc, deleteJgbLc , getWoNoList , getBasNo } from '@/api/clmanage/jgblc'
...
// 获取数据列表
const getJgbLcList = async () => {
  loading.value = true
  try {
    const res = await getJgbLc(queryParams)
    jgbLcList.value = res.data.page.list
...
const handleAdd = async() => {
  ...
  try {
    ...
    if (nextBasNo.value) {
      ...
      const res = await getBasNo('jgblc') // 注意：这里的参数可能需要根据实际业务调整
      if (res.code === 200) {
...
const handleEdit = async (row) => {
  ...
  const res = await getJgbLcById({ id: row.id })
  Object.assign(form, res.data.jgbLc)
   if (!form.writer) {
    form.writer = userStore.descr || '未知用户'
  }
  try {
