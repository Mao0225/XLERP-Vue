<template>
  <div class="jyhLc-management">
    ...
    <el-input v-model="queryParams.matRecheckNo" placeholder="请输入来料检验批次号" style="width: 200px; margin-right: 10px;"
      ...
      <el-button type="primary" @click="getJyhLcList">搜索</el-button>
    ...
    <el-table :data="jyhLcList" border v-loading="loading" style="width: 100%">
      <!-- 序号 -->
...
<el-table-column prop="writeTime" label="录入时间" />
      ...
      <el-table-column prop="surfaceQuality" label="外观、表面质量" />
  ...
  />
            ...
            <el-form-item label="外观、表面质量" prop="surfaceQuality">             
              <el-input v-model="form.surfaceQuality" placeholder="请输入外观、表面质量" />
            </el-form-item>
...
import { getJyhLc, getJyhLcById, createJyhLc, updateJyhLc, deleteJyhLc , getWoNoList , getBasNo } from '@/api/clmanage/jyhlc'
...
const rules = {
 ...
 chemCu: [
  ...
  surfaceQuality: [
    { required: true, message: '请输入外观、表面质量', trigger: 'blur' }
  ],
  size: [
...
// 获取数据列表
const getJyhLcList = async () => {
  loading.value = true
  try {
    const res = await getJyhLc(queryParams)
    jyhLcList.value = res.data.page.list
...
const handleAdd = async() => {
  ...
  try {
    ...
    if (nextBasNo.value) {
      ...
      const res = await getBasNo('jyhlc') // 注意：这里的参数可能需要根据实际业务调整
      if (res.code === 200) {
...
const handleEdit = async (row) => {
  ...
  const res = await getJyhLcById({ id: row.id })
  Object.assign(form, res.data.jyhLc)
   if (!form.writer) {
    form.writer = userStore.descr || '未知用户'
  }
  try {
