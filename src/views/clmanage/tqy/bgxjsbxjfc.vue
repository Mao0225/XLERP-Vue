<template>
  <div class="bgxjsbxjFc-management">
    ...
    <el-input v-model="queryParams.matRecheckNo" placeholder="请输入来料检验批次号" style="width: 200px; margin-right: 10px;"
      ...
      <el-button type="primary" @click="getBgxjsbxjFcList">搜索</el-button>
    ...
    <el-table :data="bgxjsbxjFcList" border v-loading="loading" style="width: 100%">
      <!-- 序号 -->
...
<el-table-column prop="writeTime" label="录入时间" />
      ...
      <el-table-column prop="testResult" label="试验结果" />
  ...
  />
            ...
            <el-form-item label="试验类型" prop="testType">
              <el-input v-model="form.testType" placeholder="请输入试验类型" />
            </el-form-item>
...
import { getBgxjsbxjFc, getBgxjsbxjFcById, createBgxjsbxjFc, updateBgxjsbxjFc, deleteBgxjsbxjFc , getWoNoList, getBasNo} from '@/api/clmanage/bgxjsbxjfc'
...
const rules = {
  ...
  testType: [
    { required: true, message: '请输入试验类型', trigger: 'blur' },
    { max: 50, message: '长度不能超过50个字符', trigger: 'blur' }
  ],
  testResult: [
...
// 获取数据列表
const getBgxjsbxjFcList = async () => {
  loading.value = true
  try {
    const res = await getBgxjsbxjFc(queryParams)
    bgxjsbxjFcList.value = res.data.page.list
...
// 获取单据号
  try {
    ...
    if (nextBasNo.value) {
      ...
      const res = await getBasNo('bgxjsbxjfc') // 注意：这里的参数可能需要根据实际业务调整
      if (res.code === 200) {
...
const handleEdit = async (row) => {
  ...
  const res = await getBgxjsbxjFcById({ id: row.id })
  Object.assign(form, res.data.bgxjsbxjFc)
   if (!form.writer) {
    form.writer = userStore.descr || '未知用户'
  }
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除数据"${row.no}"吗？`, '提示', {
    ...
    try {
      await deleteBgxjsbxjFc({ id: row.id })
