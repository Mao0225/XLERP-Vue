<template>
  <div class="jxjjLc-management">
    ...
    <el-input v-model="queryParams.matRecheckNo" placeholder="请输入来料检验批次号" style="width: 200px; margin-right: 10px;"
      ...
      <el-button type="primary" @click="getJxjjLcList">搜索</el-button>
    ...
    <el-table :data="jxjjLcList" border v-loading="loading" style="width: 100%">
      <!-- 序号 -->
...
import { getJxjjLc, getJxjjLcById, createJxjjLc, updateJxjjLc, deleteJxjjLc , getWoNoList , getBasNo } from '@/api/clmanage/jxjjlc'
...
// 获取数据列表
const getJxjjLcList = async () => {
  loading.value = true
  try {
    const res = await getJxjjLc(queryParams)
    jxjjLcList.value = res.data.page.list
...
const handleAdd = async() => {
  ...
  try {
    ...
    if (nextBasNo.value) {
      ...
      const res = await getBasNo('jxjjlc') // 注意：这里的参数可能需要根据实际业务调整
      if (res.code === 200) {
...
const handleEdit = async (row) => {
  ...
  const res = await getJxjjLcById({ id: row.id })
  Object.assign(form, res.data.jxjjLc)
   if (!form.writer) {
    form.writer = userStore.descr || '未知用户'
  }
  try {
