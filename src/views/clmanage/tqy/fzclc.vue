<template>
  <div class="fzcLc-management">
    ...
    <el-table :data="fzcLcList" border v-loading="loading" style="width: 100%">
      <!-- 序号 -->
...
// 获取数据列表
const getFzcLcList = async () => {
  loading.value = true
  try {
    const res = await getFzcLc(queryParams)
    fzcLcList.value = res.data.page.list
...
const handleEdit = async (row) => {
  ...
  const res = await getFzcLcById({ id: row.id })
  Object.assign(form, res.data.fzcLc)
   if (!form.writer) {
    form.writer = userStore.descr || '未知用户'
  }
