    ...
    <div class="action-bar">
      ...
      <div class="search-inputs">
        <el-input v-model="queryParams.contractNo" placeholder="请输入合同编号查询" style="width: 200px; margin-right: 10px;"
          clearable @clear="getRoundsteelIngotList" @keyup.enter="getRoundsteelIngotList" />
        ...
        <el-input v-model="queryParams.basno" placeholder="请输入单据号查询" style="width: 200px; margin-right: 10px;" clearable
          @clear="getRoundsteelIngotList" @keyup.enter="getRoundsteelIngotList" />
...
import { getYgPage, deleteYg, getYgById, updateStatus } from '@/api/clmanage/cl-yg'
...
const handleStatusUpdate = async (orderId, targetStatus) => {
  try {
    ...
    if (response?.code === 200) {
      ...
      getRoundsteelIngotList() // 重新加载表格数据
    } else {
...
const handleDelete = async (row) => {
  try {
    ...
    const response = await deleteYg({ id: row.id })
    if (response?.code === 200) {
