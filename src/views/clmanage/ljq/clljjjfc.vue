<template>
  <div class="clljjjfc-management">
    <div class="action-bar">
      ...
      <el-button type="primary" style="margin-left: auto;" @click="handleAdd">新增连接金具辅材</el-button>
    </div>
  ...
  />
        ...
        <el-form-item label="检测结果" prop="testResult">
          <template #default>
            ...
            />
            <div class="el-form-item__tip">提示：数值部分将自动四舍五入为整数 (单位: μm)</div>
...
import { getClljjjfcList, getClljjjfcById, createClljjjfc, updateClljjjfc, deleteClljjjfc, getGongdanByWoNo,getBasNo } from '@/api/clmanage/clljjjfc'
...
const dialogTitle = ref('新增连接金具辅材')
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await getClljjjfcList(queryParams);
...
const handleAdd = async() => {
  ...
  if (nextBasNo.value) {
    ...
    try {
      const res = await getBasNo('clljjjfc');
      if (res.code === 200) {
...
const handleEdit = async (row) => {
  dialogTitle.value = '编辑连接金具辅材';
  dialogType.value = 'edit';
  try {
    const res = await getClljjjfcById({ id: row.id });
    if (res && res.code === 200 && res.data && res.data.clLjjjFc) {
      form.value = {
        ...createEmptyForm(),
        ...res.data.clLjjjFc
      };
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除该条辅材数据吗？`, '提示', {
    ...
    await deleteClljjjfc({ id: row.id });
  ...
  formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (dialogType.value === 'add') {
        form.value.writer = userInfo.value.username;
        form.value.writeTime = formatDateTime(new Date());
        await createClljjjfc(form.value);
        ...
        await updateClljjjfc(form.value);
