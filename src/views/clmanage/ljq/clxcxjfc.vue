<template>
  <div class="clxcxjfc-management">
    ...
    <div class="action-bar">
      ...
      <el-button type="primary" style="margin-left: auto;" @click="handleAdd">新增悬垂线夹辅材</el-button>
    </div>
    <!-- 表格展示 -->
    <el-table :data="list" border v-loading="loading" style="width: 100%">
      ...
      <el-table-column prop="testResult" label="检测结果">
        <template #default="{ row }">
          <span v-if="row.maQuality === '紧固件' || row.maQuality === '预绞丝'">
            {{ formatFcResult(row.testResult) }}
          </span>
...
import { getClxcxjfcList, getClxcxjfcById, createClxcxjfc, updateClxcxjfc, deleteClxcxjfc, getGongdanByWoNo,getBasNo } from '@/api/clmanage/clxcxjfc'
...
const dialogTitle = ref('新增悬垂线夹辅材')
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await getClxcxjfcList(queryParams);
...
const handleAdd = async() => {
  ...
  if (nextBasNo.value) {
    ...
    try {
      const res = await getBasNo('clxcxjfc');
      if (res.code === 200) {
...
const handleEdit = async (row) => {
  dialogTitle.value = '编辑悬垂线夹辅材';
  dialogType.value = 'edit';
  try {
    const res = await getClxcxjfcById({ id: row.id });
    if (res && res.code === 200 && res.data && res.data.clXcxjFc) {
      form.value = {
        ...createEmptyForm(),
        ...res.data.clXcxjFc
      };
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除该条辅材数据吗？`, '提示', {
    ...
    await deleteClxcxjfc({ id: row.id });
  ...
  formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (dialogType.value === 'add') {
        form.value.writer = userInfo.value.username;
        form.value.writeTime = formatDateTime(new Date());
        await createClxcxjfc(form.value);
        ...
        await updateClxcxjfc(form.value);
