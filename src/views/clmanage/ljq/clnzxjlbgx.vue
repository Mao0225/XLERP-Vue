<template>
  <div class="clnzxjlbgx-management">
    <!-- 操作栏 -->
    <div class="action-bar">
      ...
      <el-button type="primary" style="margin-left: auto;" @click="handleAdd">新增耐张线夹铝包钢线</el-button>
    </div>
    <!-- 表格展示 -->
    <el-table :data="list" border v-loading="loading" style="width: 100%">
      ...
      <el-table-column prop="singleWireStrength" label="力学性能-单丝强度">
        <template #default="{ row }">
           {{ row.singleWireStrength ? `${row.singleWireStrength} MPa` : '' }}
        </template>
      </el-table-column>
...
<el-form-item label="单据号" prop="basno">
  ...
  />
        ...
        <el-form-item label="尺寸（mm）" prop="size">
          <el-input v-model="form.size" maxlength="50" placeholder="请输入尺寸(mm)"/>
        </el-form-item>
        <el-form-item label="力学性能-单丝强度（MPa）" prop="singleWireStrength">
          <el-input v-model="form.singleWireStrength" maxlength="50" placeholder="请输入单丝强度(MPa)"/>
        </el-form-item>
...
import { getClnzxjlbgxList, getClnzxjlbgxById, createClnzxjlbgx, updateClnzxjlbgx, deleteClnzxjlbgx, getGongdanByWoNo,getBasNo } from '@/api/clmanage/clnzxjlbgx'
...
const dialogTitle = ref('新增耐张线夹铝包钢线')
...
const rules = {
  ...
  singleWireStrength: [{ required: true, message: '请填写力学性能-单丝强度', trigger: 'blur' }],
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await getClnzxjlbgxList(queryParams);
...
const handleAdd = async() => {
  ...
  if (nextBasNo.value) {
    ...
    try {
      const res = await getBasNo('clnzxjlbgx');
      if (res.code === 200) {
...
const handleEdit = async (row) => {
  dialogTitle.value = '编辑耐张线夹铝包钢线';
  dialogType.value = 'edit';
  try {
    const res = await getClnzxjlbgxById({ id: row.id });
    if (res && res.code === 200 && res.data && res.data.clNzxjLbgx) {
      form.value = {
        ...createEmptyForm(),
        ...res.data.clNzxjLbgx
      };
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除该条铝包钢线数据吗？`, '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    await deleteClnzxjlbgx({ id: row.id });
  ...
  formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (dialogType.value === 'add') {
        form.value.writer = userInfo.value.username;
        form.value.writeTime = formatDateTime(new Date());
        await createClnzxjlbgx(form.value);
        ...
        await updateClnzxjlbgx(form.value);
