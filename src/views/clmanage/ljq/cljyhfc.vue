<template>
  <div class="cljyhfc-management">
    <div class="action-bar">
      ...
      <el-button type="primary" style="margin-left: auto;" @click="handleAdd">新增均压环屏蔽环辅材</el-button>
    </div>
    <el-table :data="list" border v-loading="loading" style="width: 100%">
      ...
      <el-table-column prop="mafactory" label="辅材制造商" />
...
<el-form-item label="单据号" prop="basno">
  ...
  />
        ...
        <el-form-item label="辅材制造商" prop="mafactory">
          <el-input
            v-model="form.mafactory"
            placeholder="请选择辅材制造商"
            readonly
...
import { getCljyhfcList, getCljyhfcById, createCljyhfc, updateCljyhfc, deleteCljyhfc, getGongdanByWoNo,getBasNo } from '@/api/clmanage/cljyhfc'
...
const dialogTitle = ref('新增均压环屏蔽环辅材')
...
const rules = {
  ...
  mafactory: [{ required: true, message: '请选择辅材制造商', trigger: 'change' }],
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await getCljyhfcList(queryParams);
...
const handleAdd = async() => {
  ...
  if (nextBasNo.value) {
    ...
    try {
      const res = await getBasNo('cljyhfc');
      if (res.code === 200) {
...
const handleEdit = async (row) => {
  dialogTitle.value = '编辑均压环屏蔽环辅材';
  dialogType.value = 'edit';
  try {
    const res = await getCljyhfcById({ id: row.id });
    if (res && res.code === 200 && res.data && res.data.clJyhFc) {
      form.value = {
        ...createEmptyForm(),
        ...res.data.clJyhFc
      };
  ...
  formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (dialogType.value === 'add') {
        form.value.writer = userInfo.value.username;
        form.value.writeTime = formatDateTime(new Date());
        await createCljyhfc(form.value);
        ...
        await updateCljyhfc(form.value);
